        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              {data.study_package?.graph_metadata && (
                <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-white/10 border border-white/10">
                  <span className={`w-1.5 h-1.5 rounded-full ${data.study_package.graph_metadata.extraction_confidence > 60 ? 'bg-green-400' : 'bg-yellow-400'} animate-pulse`} />
                  <span className="text-[10px] font-bold tracking-widest uppercase text-white/60">
                    {data.study_package.graph_metadata.extraction_confidence}% Confident
                  </span>
                </div>
              )}
            </div>
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {data.study_package?.graph_metadata && (
                <span className="text-white/10 text-[10px]">• {data.study_package.graph_metadata.triples_count} Knowledge Triples</span>
              )}
            </div>
          </div>
        </div>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      {concept.verification_status && (
                        <span className={`px-2 py-0.5 rounded-md text-[8px] font-bold tracking-widest uppercase ${
                          concept.verification_status === 'Verified' ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                        }`}>
                          {concept.verification_status}
                        </span>
                      )}
                ...
                <div className={`relative w-full h-full transition-transform duration-700 preserve-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
                  ...
                  <span className="text-[10px] font-bold tracking-widest uppercase text-black/30 mb-6 font-poppins">Discovery</span>
                    ...
                    <div className="mt-8 px-4 py-1.5 rounded-full border border-black/10 text-[10px] font-bold uppercase tracking-widest opacity-60">
                      {packageData.flashcards[flashcardIdx]?.is_graph_grounded ? 'Source Grounded' : 'AI Synthetic'} • {packageData.flashcards[flashcardIdx]?.tags?.find(t => t.startsWith('Confidence:'))?.split(': ')[1] || 'Verified'}
                    </div>
                  </div>
                </div>
              </div>
