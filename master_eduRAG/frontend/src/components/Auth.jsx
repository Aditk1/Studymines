...
export default function Auth({ onLogin }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Archive connection failed')
    } finally {
      setLoading(false)
    }
  }
        ...
        <div className="relative z-10">
          <header className="text-center mb-10">
            ...
            <h1 className="text-4xl font-medium tracking-tighter mb-2">
              {isLogin ? 'Welcome to ' : 'Join the '}
              <em className="text-white/60">Academy</em>
            </h1>
            <p className="text-white/40 text-sm">Synchronize your cognitive archive.</p>
          </header>
